...
int wiringCPII2CWriteReg8 (int fd, int reg, int value)
{
  union i2c_smbus_data data ;
...
int wiringCPII2CWriteReg16 (int fd, int reg, int value)
{
  union i2c_smbus_data data ;
...
int wiringCPII2CSetupInterface (const char *device, int devId)
{
  int fd ;

  if ((fd = open (device, O_RDWR)) < 0)
    return wiringCPIFailure (WPI_ALMOST, "Unable to open I2C device: %s\n", strerror (errno)) ;

  if (ioctl (fd, I2C_SLAVE, devId) < 0)
    return wiringCPIFailure (WPI_ALMOST, "Unable to select I2C device: %s\n", strerror (errno)) ;
...
int wiringCPII2CSetup (const int devId)
{
  int rev ;
  const char *device ;
  ...
  return wiringCPII2CSetupInterface (device, devId) ;
}
